...
class v_4_0_6 extends \phpbb\db\migration\migration
{

	public function effectively_installed()
	{
//		return isset($this->config['thanks_for_posts_version']) && version_compare($this->config['thanks_for_posts_version'], '4.0.6', '>=');
        return false; // Always process this upgrade, as it checks every step
    }
...
// Migrate all old notifications
	...
	public function get_needed_modules_array()
	{
		...
		$update_sql = "UPDATE " . MODULES_TABLE . "
			SET module_auth =  REPLACE(module_auth, 'gfksx', 'naguissa')
...
// Add configs
			...
			array('if', array(
					...
					array('config.add', array('thanks_for_posts_version', '4.0.6'))
				)
			),
			array('config.update', array('thanks_for_posts_version', '4.0.6')),
		);
